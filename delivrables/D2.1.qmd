...
The videos can be accessed through the following link: [https://wp6.techtraplastice.eu/wp2-portfolio.html](https://wp6.techtraplastice.eu/wp2-portfolio.html)


![Videos to support the common understanding of the Questionnaires.](figures/2.1/Videos.png){#fig-videos width="75%" fig-align="center"}
...
The timeline for respond the questionnaires was between **September 20206 - Nov 2026**.
...
1. Services (Mid Sept 2026 )
2. Capacities (Oct 2026)
3. Developments (Mid Oct 2026)
4. Training (November 2026)
...
![Presential workshop on the Canva of at the meeting consortium in Chile Nov 2026](figures/2.1/Chile.jpg){#fig-chile width="60%" fig-align="center"}
...
# Global data
datos <- read_excel(here("delivrables/tables/2.1/Developments - Responses.xlsx"))
#datos <- read_excel("figures/1.3/Data-USACH.xlsx", sheet = "total")


datos <- 
  datos %>% select(HEI, Title, Subtitle) %>%   
  mutate(Subtitle = tolower(Subtitle))  %>% 
  rowid_to_column("ID") %>% 
  select(HEI, ID, Title, Subtitle) %>% 
  arrange(HEI) %>%
  mutate_all(linebreak) %>%
...
# Global data
datos <- 
  read_excel(here("delivrables/tables/2.1/Capabilities - Responses.xlsx"))  %>% 
  select(1,3,5)  %>% 
   arrange(desc(Universidad))
colnames(datos) <- c("HEI", "Title", "Description")

# datos <- 
#   datos %>% 
#   mutate(
#     Title = replace(Title, str_detect(Title, '&'), c(" and ")),
#     Description = replace(Description, str_detect(Description, '&'), c(" and "))
#         )
...
## Technical Services at each HEI's
...
#View(datos)
datos <- 
  read_excel(here("delivrables/tables/2.1/Technical Services - Responses.xlsx"))  %>% 
  select(1,3,5, 11)  %>% 
   arrange(desc(Universidad))

colnames(datos) <- c("HEI", "Title", "Description", "Aplication")
...
#View(datos)
datos <- 
  read_excel(here("delivrables/tables/2.1/Training - Responses.xlsx"))  %>% 
  select(1,3,5, 11)     

colnames(datos) <- c("HEI", "Title", "Description", "Level")
...
On the other hand, complementary qualitative data was obtained through the canvas-based workshop carried out during the consortium meeting in Chile. 
This includes visual records of the completed canvases, which capture collective reflections, institutional insights, and identified opportunities for collaboration. 
These materials provide additional context and depth to the structured dataset and are available through the following link: 


:::{layout="[[100],[48,-3,48]]"}

![Example of canva discussed at the consortium meeting in Chile in Nov. 2026](figures/2.1/Canva-00.jpg){#fig-canva width="60%" fig-align="center"}

![Workshop developed by each HEI's](figures/2.1/Canva-01.jpg){#fig-canva1 width="60%" fig-align="center"}

![Feedback made by the consortium meeting at Chile in Nov. 2026](figures/2.1/Canva-02.jpg){#fig-canva2 width="60%" fig-align="center"}

:::

Together, these sources constitute a comprehensive and multi-layered dataset, enabling both systematic analysis and contextual interpretation of the technology transfer capacities within the consortium.
...
# Complete raw data collected throughrout T2.1 {#sec-A2}
